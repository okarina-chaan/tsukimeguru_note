...
## 開発環境
- Ruby on Rails 8を使用したWebアプリケーションです。
- フロントエンドにはTailwind CSSとJavaScript（esbuild経由）が使用されています。
- データベースは本番環境でPostgreSQL、開発・テスト環境でSQLite3が使用されています。
- RSpecとCapybaraを使用したテストが含まれています。
- Dockerを使用して開発環境を構築できます。

## コードについて
- コードベースはMVCアーキテクチャに従っています。
- コードのコメントについては日本語で記述します。分かりにくい関数名についてはコメントせず、コードの中でわかりやすい関数名を使用してください。ひと目見て分かりにくいロジックの流れについて簡単に書きます。
