...
RSpec.describe LineNotificationService do
    ...
    it "新月のときは新月のMoon Noteの日のメッセージが返る" do
      expect(LineNotificationService.message_for(:new_moon)).to include("新月のMoon Noteの日")
    end

    it "満月のときは満月のMoon Noteの日のメッセージが返る" do
      expect(LineNotificationService.message_for(:full_moon)).to include("満月のMoon Noteの日")
    end

    it "上弦の月のときは上弦の月のMoon Noteの日のメッセージが返る" do
      expect(LineNotificationService.message_for(:first_quarter_moon)).to include("上弦の月のMoon Noteの日")
    end

    it "下弦の月のときは下弦の月のMoon Noteの日のメッセージが返る" do
      expect(LineNotificationService.message_for(:last_quarter_moon)).to include("下弦の月のMoon Noteの日")
    end

    it "不明なフェーズのときはnilが返る" do
      expect(LineNotificationService.message_for(:unknown_phase)).to be_nil
    end
  end
  ...
  describe ".notify" do
    ...
    it "line_user_idがあるユーザーにpush_messageが呼ばれる" do
      ...
      expect(mock_client).to have_received(:push_message).with("LINE123", anything)
    end
